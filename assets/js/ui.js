...
(function (global) {
  ...
  var ICONS = {
    ...
    medical: '<path d="M9 4.5h6v4.5h4.5v6H15v4.5H9V15H4.5V9H9z"/>',
    grain: '<path d="M12 21c-4.2 0-7.2-2.6-7.2-6.8 0-4.7 7.2-9.2 7.2-9.2s7.2 4.5 7.2 9.2c0 4.2-3 6.8-7.2 6.8z"/><path d="M8.6 16.3c1 1 5.8 1 6.8 0M9.8 13.4c.7.7 3.7.7 4.4 0"/>',
    clock: '<circle cx="12" cy="12" r="8.5"/><path d="M12 7.5V12l3 2.2"/>',
    mail: '<rect x="3.5" y="5.5" width="17" height="13" rx="2.2"/><path d="m4.5 7.5 7.5 6 7.5-6"/>',
    cam: '<rect x="3" y="6.5" width="18" height="12" rx="2.6"/><path d="m8 6.5 1.5-2.7h5L16 6.5"/><circle cx="12" cy="12.5" r="3.1"/>',
  ...
  function drawer(opt) {
    ...
    d.innerHTML = '<div class="modal-hd"><h3>' + esc(opt.title || '详情') + '</h3>' +
      ...
      (opt.html || '') + '</div>' + (opt.actions ? '<div class="modal-ft"></div>' : '');
  ...
  /* ---------- 气泡菜单（自动避让屏幕边缘） ---------- */
  var curPop = null;
  function popover(anchor, html, onMount, opts) {
    closePop();
    var p = document.createElement('div');
    p.className = 'pop'; p.innerHTML = html;
    document.body.appendChild(p);
    if (opts && opts.width) p.style.width = opts.width + 'px';
  ...
  function table(opt) {
    ...
    if (!rows.length) return '<div class="empty"><span>🗂️</span>' + esc(opt.empty || '暂无数据') + '</div>';
    ...
    view.forEach(function (row, i) {
      ...
      if (opt.selectable) h += '<td><input type="checkbox" data-check="' + esc(row.id) + '"' + (opt.selected && opt.selected[row.id] ? ' checked' : '') + ' aria-label="选择该行"></td>';
      cols.forEach(function (c) {
    ...
    } else if (ps) { h += '<div class="pager"><span>共 ' + rows.length + ' 条</span></div>'; }
    return h;
  }
  ...
  function formHtml(fields, grid) {
    ...
    fields.forEach(function (f) {
      ...
      if (f.type === 'switch') {
        h += '<div class="' + cls + '"><label class="switch"><input type="checkbox" data-f="' + f.name + '"' + (f.value ? ' checked' : '') + '><i></i><span>' + esc(f.label) + '</span></label>' + (f.hint ? '<p class="field-hint">' + esc(f.hint) + '</p>' : '') + '</div>';
        return;
      }
      h += '<div class="' + cls + '"><label for="' + id + '">' + esc(f.label) + (f.required ? ' <b>*</b>' : '') + '</label>';
      if (f.type === 'select') {
        h += '<select id="' + id + '" data-f="' + f.name + '"' + (f.disabled ? ' disabled' : '') + '>' + (f.options || []).map(function (o) {
        ...
        h += '<textarea id="' + id + '" rows="' + (f.rows || 3) + '" data-f="' + f.name + '" placeholder="' + esc(f.placeholder || '') + '">' + esc(f.value || '') + '</textarea>';
      } else {
        h += '<input id="' + id + '" type="' + (f.type || 'text') + '" data-f="' + f.name + '" value="' + esc(f.value == null ? '' : f.value) + '" placeholder="' + esc(f.placeholder || '') + '"' + (f.min != null ? ' min="' + f.min + '"' : '') + (f.max != null ? ' max="' + f.max + '"' : '') + (f.step != null ? ' step="' + f.step + '"' : '') + (f.disabled ? ' disabled' : '') + '>';
      }
  ...
  function formRead(root) {
    ...
    [].forEach.call(root.querySelectorAll('[data-f]'), function (el) {
      ...
      o[n] = el.type === 'checkbox' ? el.checked : (el.type === 'number' ? (el.value === '' ? '' : +el.value) : String(el.value).trim());
    });
    return o;
  }
  ...
  function copy(text) {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text).then(function () { toast('已复制', 'ok'); }, function () { fallback(); });
    } else fallback();
    function fallback() {
