/* =====================================================================
 * 寒羽智瞳 · 数据层（v2）
 * 数据保存在浏览器 localStorage；接入后端时把 load/save 换成 API 调用。
 * ===================================================================*/
(function (global) {
  'use strict';
  ...
  function seed() {
    ...
    /* 死淘记录：近 90 天，校准到 294 只 → 累计死淘率 ≈ 2.3% */
    ...
    db.messages = [
      ...
      { id: uid('msg'), title: '平台升级至 v1.0.0', body: '新增视频监控与设备管理模块，预警阈值支持分阶段配置。', ts: tstr(new Date(now - 12 * 864e5)), read: true, kind: 'info' }
    ];
  ...
  function load() {
    ...
    try {
      ...
      if (raw) { var o = JSON.parse(raw); if (o && o.version === 2) { _db = o; return _db; } }
    } catch (e) { /* 数据损坏则重建 */ }
    _db = seed(); save();
    return _db;
  }
  ...
  function kpi() {
    ...
    return {
      ...
      camOn: cam.on, camTotal: cam.total, camAi: cam.ai,
      devOn: dev.on, devTotal: dev.total, devOff: dev.off, devLow: dev.low
    };
  }
  ...
  function camStats() {
    ...
    return {
      total: cams.length,
      on: cams.filter(function (c) { return c.online; }).length,
      off: cams.filter(function (c) { return !c.online; }).length,
      ai: cams.filter(function (c) { return c.detect; }).length
    };
  }
  function devStats() {
    ...
    return {
      total: ds.length,
      on: ds.filter(function (x) { return x.online; }).length,
      off: ds.filter(function (x) { return !x.online; }).length,
      low: ds.filter(function (x) { return x.online && x.power < 25; }).length
    };
  }
  function listCams() {
    var db = load();
    return (db.cams || []).map(function (c) {
      ...
      return {
        id: c.id, houseCode: c.houseCode, zone: c.zone, online: c.online,
        res: c.res, fps: c.fps, detect: c.detect, photo: c.photo,
        farmId: h.farmId, batch: h.batch || '', keeper: h.keeper || '', breed: h.breed || ''
      };
    });
  }
